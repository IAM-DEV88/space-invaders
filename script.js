...
class Enemy {
  ...
  update(x, y) {
    ...
    this.game.projectilePool.forEach((projectile) => {
      if (
        !projectile.free &&
        this.game.checkCollission(this, projectile) &&
        this.lives > 0
      ) {
        this.hit(1);
        projectile.reset();
    ...
    // check collisions enemies players
    if (this.game.checkCollission(this, this.game.player) && this.lives > 0) {
      this.lives = 0;
...
class Boss {
  ...
  update() {
    ...
    this.x += this.speedX;
    this.y += this.speedY;
    // collision detection for boss/projectiles
    this.game.projectilePool.forEach((projectile) => {
      if (
        this.game.checkCollission(this, projectile) &&
        !projectile.free &&
    ...
    // collision detection for boss/player
    if (this.game.checkCollission(this, this.game.player) && this.lives > 0) {
      this.game.gameOver = true;
      this.lives = 0;
    }
...
class Game {
  ...
  drawStatusText(context) {
    ...
    context.shadowColor = "black";
    context.fillText("Puntaje: " + this.score, 20, 40);
    context.fillText("Oleada: " + this.waveCount, 20, 80);
    ...
    if (this.gameOver) {
      ...
      context.fillText("NAVE DESTRUIDA", this.width * 0.5, this.height * 0.5);
